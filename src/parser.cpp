...
std::vector<std::shared_ptr<FunctionRefNode>> parse_params(std::vector<Tokens> &tokens)
{
    ...
    {
        params.push_back(parse_function_ref(tokens).value());
...
std::optional<std::shared_ptr<FunctionRefNode>> parse_function_ref(std::vector<Tokens> &tokens)
{
    ...
    auto ret = match_and_remove({TokenType::Integer}, tokens);
    // auto f = std::make_shared<FunctionRefNode>(name.value(), params, ret.value());
    // return f;
    return {};
}
